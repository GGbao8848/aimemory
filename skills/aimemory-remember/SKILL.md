...
每台装了采集器的机器会**自动**把会话归档并沉淀出记忆（L0 → L1 摘要 → L2 事实，全程无需任何人动手）。
因此**日常对话不需要 add_memory**——后台会沉淀一切。只在这些情况才主动提交：

- 用户明确说"记住/别忘了"；
- 需要即时生效的关键决策/约束（后台链路有分钟级延迟）；
- 内容不会进入任何被归档的会话（如口头随聊的偏好）。

重复提交同一事实是安全的（服务端消解层会判 NOOP 拦下），但整段对话双投浪费判定 token——按上面的分工来。
