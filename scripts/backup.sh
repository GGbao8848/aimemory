#!/bin/bash
# aimemory 备份脚本：SQLite 在线备份 + L0/L3 目录打包 + 校验和 + 保留策略
# 用法: scripts/backup.sh [数据目录] [备份输出目录] [保留份数]
#   数据目录默认 <仓库>/data（含 aimemory.db、l0/、l3/）；保留份数默认 7
...
for d in l0 l3; do
  if [ -d "$DATA_DIR/$d" ]; then
    tar -czf "$DEST/$d.tar.gz" -C "$DATA_DIR" "$d"
    echo "  ✓ $d.tar.gz ($(du -h "$DEST/$d.tar.gz" | cut -f1))"
  else
    echo "  - $DATA_DIR/$d 不存在，跳过"
  fi
done
