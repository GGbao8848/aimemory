...
async function handle(req, res, session) {
  ...
  try {
    ...
    if (!res.headersSent) {
      res.status(500).json({ jsonrpc: '2.0', error: { code: -32603, message: `内部错误: ${e.message}` } });
    } else {
      res.end();
    }
  }
...
async function createSession(req, res) {
  ...
  if (!userId) {
    res.status(401).json({ jsonrpc: '2.0', error: { code: -32000, message: '缺少或无效的 API Key：请在 Authorization 头携带 Token m0-xxx' } });
    return;
  }
